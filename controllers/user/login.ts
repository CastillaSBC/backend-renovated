...
export default async function login(req: Request, res: Response) {
    ...
    Logger.warning(
        `Last date: ${lastDateInSeconds} seconds, streak: ${reFetchedUser!.DailyStreak!.streak
        }`
    );
    ...
    if (lastDateInSeconds >= 172800) {
        ...
        Logger.success(
            `${reFetchedUser!.username
            }'s daily streak has been reset due to more than 48 hours have passed`
        );
    }
    ...
    if (lastDateInSeconds >= 86400) {
        ...
        if (diff < 2) {
            Logger.warning(
                ...
                } is getting a new daily streak because their last daily streak was ${Math.floor(
                    diff / (1000 * 60 * 60 * 24)
                )} days ago`
            );
            await prisma.dailyStreak.update({
